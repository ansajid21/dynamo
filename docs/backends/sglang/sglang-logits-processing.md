...
The quick test targets aggregated deployments. In disaggregated mode the prefill worker emits one token before decode resumes. The Backend SDK skips the test hook on the prefill role (the shared generation-stage gating returns no entries there), but the decode-side output can still be affected by the prefill-produced leading token. Use aggregated mode to verify the wiring.

### How the Backend SDK wires this up

The unified SGLang engine threads logits processors through the shared spec layer in `dynamo.backend._engine` and the per-backend realizer at `dynamo.sglang.logits_processing.adapter`:
