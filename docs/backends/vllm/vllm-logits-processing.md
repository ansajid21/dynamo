...
- **Shared spec layer**: The engine declares a backend-neutral `LogitsProcessorSpec` (see `dynamo.backend._engine`). The shared `logits_processors_for_request` helper owns the generation-stage gating (activate only on `AGGREGATED` / `DECODE`) and the per-request freshness policy.
...
The quick test targets aggregated deployments. In disaggregated mode the prefill worker emits one token before decode resumes, and the test processor has per-request state. The Backend SDK skips the test hook on the prefill role (the shared generation-stage gating returns no entries there), but the decode-side output can still be affected by the prefill-produced leading token. Use aggregated mode to verify the wiring.

### How the Backend SDK wires this up

The unified vLLM engine threads logits processors through the shared spec layer in `dynamo.backend._engine` and the per-backend realizer at `dynamo.vllm.logits_processing.adapter`:
