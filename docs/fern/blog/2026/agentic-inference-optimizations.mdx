...
![Cumulative cache reads vs writes across a 42-call Claude Code session. Cache reads (891K tokens) grow steeply while writes (76K) and uncached input stay flat -- an 11.7x read/write ratio.](../../digest/agentic-inference/cumulative-reads-writes.png)
...
![Where nvext fits in the agentic protocol stack alongside MCP and A2A](../../digest/agentic-inference/protocol-stack.svg)
...
![How priority flows from harness through router dispatch to engine treatment](../../digest/agentic-inference/two-gates.svg)
...
![KV cache memory hierarchy: GPU (HBM) at ~ns, CPU (pinned DRAM) at ~us, Local NVMe at ~ms, Remote Storage (NIXL) at ~ms via RDMA](../../digest/agentic-inference/kv-memory-hierarchy.svg)
...
![During tool calls, KV blocks offload to host memory and storage, then prefetch back to GPU before the next LLM call.](../../digest/agentic-inference/tool-call-offload-prefetch.svg)
...
![Lead agent conversation flow branches to a sub-agent. The sub-agent's ephemeral KV is evicted on session end.](../../digest/agentic-inference/subagent-lifecycle-vertical.svg)
