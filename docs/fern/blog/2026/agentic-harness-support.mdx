...
sidebar-title: Agentic Harness Support
...
These changes build on the performance considerations outlined in our [first post](agentic-inference-optimizations.mdx), which focused on the serving architecture underneath agentic inference: the frontend, router, and KV cache management. This follow-up focuses on correctness, user-experience equivalence, and performance.
...
![Standard server vs Dynamo across two turns of an agent loop. Each turn crosses the network at two edges: harness-to-server (where prompt stability decides cache reuse) and server-to-harness (where streaming tool dispatch decides when the harness can act). Dynamo changes both edges.](../../digest/agentic-inference/images/fig-1-agent-loop.svg)
...
![TTFT across three prompt-prefix conditions on a 52K-token prompt: a stable prefix lands at 168 ms, the stripped Anthropic preamble at 169 ms, while a varying per-session billing header pushes TTFT to 912 ms — a single token at position zero is the difference between hot KV reuse and a full cold prefill.](../../digest/agentic-inference/images/fig-2-ttft-prefix-stability.svg)
