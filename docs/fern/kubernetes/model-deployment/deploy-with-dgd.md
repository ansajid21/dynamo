...
For the concepts behind the CRDs and the operator, see the [Model Deployment](overview.mdx) and the [API Reference](../../reference/kubernetes-api/additional-resources/api-reference-k8s.md).
...
For disaggregation-specific configuration — RDMA resources, UCX environment variables, and prefill/decode scaling — see [Disaggregated Serving](disaggregated-serving.md).
...
<CardGroup cols={2}>
  ...
  <Card title="Set up KV Cache Offloading" icon="regular layer-group" href="kv-cache-offloading.mdx">
    Spill KV blocks to host memory or disk to serve longer contexts and reuse cache.
  </Card>
  <Card title="Set up Disaggregated Serving" icon="regular arrows-split-up-and-left" href="disaggregated-serving.md">
    Split prefill and decode into independently scaled workers with KV transfer over RDMA.
  </Card>
  <Card title="Sizing with AIConfigurator" icon="regular ruler-combined" href="sizing-with-aiconfigurator.mdx">
