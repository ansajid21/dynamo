...
This is intentionally not the same lifecycle model as the operator-managed GAIE quickstart. There is no `DynamoGraphDeployment`, no Dynamo worker runtime, and no Dynamo event plane in this on-ramp. We will be starting with a simple Kubernetes `apps/v1` Deployment of `vllm serve` pods.
...
- Baseline Gateway API knowledge from the upstream
  ...
  and the upstream
  [Inference Extension GAIE guide](https://github.com/kubernetes-sigs/gateway-api-inference-extension/blob/main/site-src/guides/index.md).
- A Gateway API implementation that supports GAIE `InferencePool` resources and `endpointPickerRef`
  ...
  for other implementations. The example below uses the [AgentGateway](https://agentgateway.dev/docs/kubernetes/main/quickstart/install/) in the `agentgateway-system` namespace.
- A model namespace that can run GPU workloads.
- Hugging Face credentials if the model requires them. The
  [Kubernetes Quickstart](quickstart.mdx) explains the general secret
...
yet. Follow the instructions for your gateway (e.g.
...
# The Gateway is created in the `agentgateway-system` namespace, and its
# listener allows routes from other namespaces (allowedRoutes.namespaces.from:
# All) so the HTTPRoute in your workload namespace attaches.
...
# The Gateway lives in the `agentgateway-system` namespace:
