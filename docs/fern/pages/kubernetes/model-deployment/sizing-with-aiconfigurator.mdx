...
DGD. For that workflow, see [Sizing a Local Deployment with AIConfigurator](../../cli/model-deployment/sizing-with-aiconfigurator.mdx).
</Note>
...
- A DGD in progress from [Deploy with DGD](deploy-with-dgd.md), with the Frontend and worker defined and only parallelism left to set.
...
- Return to [Deploy with DGD](deploy-with-dgd.md) to apply the deployment and send a request.
