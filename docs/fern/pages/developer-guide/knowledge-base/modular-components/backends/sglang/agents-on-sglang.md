...
This guide covers SGLang-specific configuration for agentic serving with Dynamo. It explains which SGLang engine flags to enable, how Dynamo's [agent hints](../../../../additional-resources/nvidia-request-extensions-nvext.md#agent-hints) map to SGLang behavior, and how to use session control to manage KV cache for multi-turn agent conversations.
...
Dynamo's agent hints give the router per-request metadata. SGLang's engine flags control how that metadata affects scheduling and eviction on the worker. For the cross-layer Dynamo priority semantics, see [Priority Scheduling](../../../../../features/agents/priority-scheduling.md).
...
[Router Configuration and Tuning](../../router/configuration-and-tuning.md#routing-behavior).
...
- **[NVIDIA Request Extensions (nvext)](../../../../additional-resources/nvidia-request-extensions-nvext.md)**: Full `nvext` field reference including agent hints
- **[Configuration and Tuning](../../router/configuration-and-tuning.md)**: Router configuration and CLI arguments
- **[SGLang HiCache](hicache.md)**: Enabling hierarchical KV cache
