...
> [!NOTE]
> 也提供包含所有依赖项的预构建容器。请参阅 [Release Artifacts](../../../../../reference/general/release-artifacts.mdx) 了解容器镜像。
...
| 内存管理 | Dynamo KV Block Manager、[LMCache](../../../../../cli/model-deployment/kv-cache-offloading.mdx)、[SGLang HiCache](../../../../../developer-guide/knowledge-base/modular-components/backends/sglang/hicache.md)、[FlexKV](../../../../../cli/model-deployment/kv-cache-offloading.mdx) |
| 网络和存储 | Mooncake、DOCA NetIO、GDS、POSIX、S3、3FS（[通过 NIXL 支持](../../../../../developer-guide/knowledge-base/modular-components/kvbm/kvbm-design.md)） |
...
- [KV cache-aware routing](../../../../../developer-guide/knowledge-base/modular-components/router/router-design.md) 根据 worker 负载和现有缓存命中情况智能路由请求。通过复用预先计算的 KV 对，它绕过 prefill 计算，立即开始 decode 阶段。[Baseten](https://www.baseten.co/blog/how-baseten-achieved-2x-faster-inference-with-nvidia-dynamo/#how-baseten-uses-nvidia-dynamo) 应用 Dynamo KV 缓存感知路由后，在 Qwen3 Coder 480B A35B 上实现了 2 倍更快的 TTFT 和 1.6 倍吞吐量。

- [KV cache offloading](../../../../../developer-guide/knowledge-base/modular-components/kvbm/kvbm-design.md) 通过将 KV 缓存从 HBM 移动到主机内存、本地磁盘或远程存储等成本更低的存储层，扩展可用上下文窗口。复用预计算状态可改善 TTFT、降低总体拥有成本（TCO），并支持更长上下文处理。
...
Dynamo 提供内置指标、分布式追踪和日志，用于监控推理部署。请参阅 [Observability Guide](../../../../../kubernetes/installation/observability.md) 了解设置详情。
...
- [KV Cache-Aware Routing](../../../../../developer-guide/knowledge-base/modular-components/router/router-guide.md) -- 配置智能请求路由
- [KV Cache Offloading](../../../../../developer-guide/knowledge-base/modular-components/kvbm/kvbm-guide.md) -- 设置多层内存管理
- [Planner](../../developer-guide/knowledge-base/modular-components/planner/planner-guide.md) -- 配置基于 SLA 的自动扩缩容
- [Kubernetes Deployment](../../../../../kubernetes/getting-started/quickstart.mdx) -- 使用 Grove 进行大规模部署
- [Overall Architecture](../../developer-guide/knowledge-base/overview.md) -- 完整技术设计
- [Support Matrix](../../../../../reference/general/compatibility.mdx) -- 检查硬件和引擎兼容性

**延伸阅读：** [Dynamo Digest](../../../../../blog/latest.mdx)。
