...
The vLLM-Omni backend uses a dedicated entrypoint: `python -m dynamo.vllm.omni`. This page documents its CLI flags. For deployment guides and per-modality examples, see the [Diffusion Overview](../../features/diffusion/overview.md).

<Note>
These are the vLLM-Omni orchestration flags. The omni worker also accepts the same underlying vLLM engine arguments and the cross-cutting [Dynamo Runtime](../components/runtime-configuration.mdx) flags (`--namespace`, `--endpoint`, and others).
</Note>
...
See [vLLM-Omni Disaggregated Serving](../../developer-guide/knowledge-base/modular-components/backends/vllm/vllm-omni-disaggregated-serving.md) for usage.
