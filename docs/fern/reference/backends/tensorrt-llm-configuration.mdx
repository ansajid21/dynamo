...
_These flags configure the experimental diffusion pipeline (video and image generation). They are only relevant when `--modality` is set to `video_diffusion` or `image_diffusion`. See [TensorRT-LLM Diffusion](../../features/diffusion/overview.md) for usage details._
...
_These flags set the default values applied to video and image generation requests that do not specify the corresponding parameter. They are only relevant when `--modality` is `video_diffusion` or `image_diffusion`. See [TensorRT-LLM Diffusion](../../features/diffusion/overview.md) for usage details._
