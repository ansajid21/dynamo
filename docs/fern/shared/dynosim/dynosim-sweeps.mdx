...
local CLI deployment. Use a sweep after a single [DynoSim run](dynosim-replay.mdx) works and you want to identify
...
2. Exercise the live frontend and router with [Simulate a Local Deployment](../../cli/operations/dynosim/mocker-live-simulation.mdx) or
   [Simulate a Kubernetes Deployment](../../kubernetes/operations/dynosim/mocker-live-simulation.mdx).
3. Deploy the candidate on the target hardware and benchmark it with AIPerf using either the
   [Kubernetes workflow](../../kubernetes/operations/benchmarking-with-aiperf.mdx) or the [local CLI workflow](../../cli/operations/benchmarking-with-aiperf.mdx).
</Step>
</Steps>
