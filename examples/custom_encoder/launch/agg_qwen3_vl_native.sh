...
# Aggregated Qwen3-VL with an in-process custom vision encoder.
...
MODEL="${DYN_MODEL:-Qwen/Qwen3-VL-2B-Instruct}"
ENCODER_CLASS="${DYN_ENCODER_CLASS:-examples.custom_encoder.qwen3_vl_native_encoder.Qwen3VLNativeEncoder}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            cat <<'EOF'
Usage: agg_qwen3_vl_native.sh [OPTIONS]

Options:
  --model <id>           Qwen3-VL checkpoint
...
print_launch_banner --multimodal "Qwen3-VL CustomEncoder — Native Aggregated" \
    "$MODEL" "$HTTP_PORT" \
...
echo "[2/2] Starting Qwen3-VL worker (model=$MODEL, GPU=$WORKER_GPU)..."
