...
| **[文档](https://docs.nvidia.com/dynamo/)** | **[路线图](https://github.com/ai-dynamo/dynamo/issues/5506)** | **[配方](https://github.com/ai-dynamo/dynamo/tree/main/recipes)** | **[示例](https://github.com/ai-dynamo/dynamo/tree/main/examples)** | **[预构建容器](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-dynamo/collections/ai-dynamo)** | **[摘要](docs/fern/blog/latest.mdx)** | **[设计提案](https://github.com/ai-dynamo/dynamo/issues?q=is%3Aissue+label%3A%22dep%3Adraft%22%2C%22dep%3Aproposed%22%2C%22dep%3Aapproved%22%2C%22dep%3Aimplementing%22%2C%22dep%3Acompleted%22%2C%22dep%3Adeferred%22%2C%22dep%3Asuperseeded%22)** | **[如何贡献](#社区与贡献)** |
...
|---|:----:|:----------:|:--:|
| [**分离式服务**](docs/fern/developer-guide/knowledge-base/concepts/system-architecture/disaggregated-serving.md) | ✅ | ✅ | ✅ |
| [**KV 感知路由**](docs/fern/developer-guide/knowledge-base/modular-components/router/overview.md) | ✅ | ✅ | ✅ |
| [**基于 SLA 的 Planner**](docs/fern/developer-guide/knowledge-base/modular-components/planner/planner-guide.md) | ✅ | ✅ | ✅ |
| [**KVBM**](docs/fern/developer-guide/knowledge-base/modular-components/kvbm/overview.md) | 🚧 | ✅ | ✅ |
| [**多模态**](https://docs.nvidia.com/dynamo/user-guides/multimodal) | ✅ | ✅ | ✅ |
| [**工具调用**](docs/fern/features/tool-calling-and-reasoning/tool-call-parsing.mdx) | ✅ | ✅ | ✅ |
...
**[架构深入解析 →](docs/fern/developer-guide/knowledge-base/overview.md)**

### 核心能力

| 能力 | 作用 | 价值 |
|------|------|------|
| [**分离式预填充/解码**](docs/fern/developer-guide/knowledge-base/concepts/system-architecture/disaggregated-serving.md) | 将预填充和解码拆分为可独立扩缩容的 GPU 池 | 最大化 GPU 利用率；每个阶段都运行在针对其工作负载调优的硬件上 |
| [**KV 感知路由**](docs/fern/developer-guide/knowledge-base/modular-components/router/overview.md) | 根据 worker 负载和 KV 缓存重叠度路由请求 | 消除冗余预填充计算，TTFT 快 2x |
| [**KV Block Manager (KVBM)**](docs/fern/developer-guide/knowledge-base/modular-components/kvbm/overview.md) | 在 GPU → CPU → SSD → 远程存储之间卸载 KV 缓存 | 将有效上下文长度扩展到 GPU 显存之外 |
| [**ModelExpress**](https://github.com/ai-dynamo/modelexpress) | 通过 NIXL/NVLink 在 GPU 之间流式传输模型权重 | 新副本冷启动快 7x |
| [**Planner**](docs/fern/developer-guide/knowledge-base/modular-components/planner/planner-guide.md) | 由 SLA 驱动的自动扩缩容器，可分析工作负载并调整资源池规模 | 以最低总体拥有成本（TCO）满足延迟目标 |
...
| [**容错**](docs/fern/kubernetes/fault-tolerance/request-migration.md) | 金丝雀健康检查 + 运行中请求迁移 | worker 可以失败，但用户请求不应失败 |
...
> **注意：** TensorRT-LLM 需要配合 `--extra-index-url https://pypi.nvidia.com` 使用 `pip`。TRT-LLM 专用说明请参阅[安装指南](docs/fern/cli/installation/install-dynamo.mdx)。

然后按上面的方式启动 frontend 和一个 worker。系统依赖和后端专用说明请参阅[完整安装指南](docs/fern/cli/installation/install-dynamo.mdx)。
...
完整列表见 [recipes/](recipes/README.md)。云平台专用指南：[AWS EKS](docs/fern/kubernetes/installation/managed-kubernetes/eks/eks-setup.mdx) · [Google GKE](docs/fern/kubernetes/installation/managed-kubernetes/gcp/gke-setup.mdx) · [Azure AKS](docs/fern/kubernetes/installation/managed-kubernetes/azure/aks-setup.mdx) · [Amazon ECS](docs/fern/kubernetes/installation/managed-kubernetes/eks/ecs.mdx)

## 从源码构建

适用于希望在本地构建和开发的贡献者。详情请参阅[完整构建指南](docs/fern/developer-guide/advanced-customizations/building-from-source.md)。
...
- **[贡献指南](docs/fern/developer-guide/contributing/overview.md)** — 如何贡献代码、文档和配方
...
- **[基准测试指南](docs/fern/recipes/feature-benchmarks/benchmarking-guide.md)** – 使用 AIPerf 比较部署拓扑
- **[SLA 驱动部署](docs/fern/developer-guide/knowledge-base/modular-components/planner/planner-guide.md)** – 优化部署以满足 SLA 要求
...
Dynamo 使用 TCP 进行组件间通信。在 Kubernetes 上，原生资源（[CRDs + EndpointSlices](docs/fern/developer-guide/knowledge-base/kubernetes/kubernetes-operator/service-discovery.md)）负责服务发现。对大多数部署来说，外部服务是可选的：
