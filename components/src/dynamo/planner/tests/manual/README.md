...
1. Run pre-deployment profiling for your specific setup. See the [pre-deployment profiling documentation](../../../../../../docs/fern/developer-guide/knowledge-base/modular-components/profiler/profiler-guide.md) for detailed instructions.
...
1. **Unit Tests** (`components/src/dynamo/planner/tests/unit/test_load_based_scaling.py` + plugin/orchestrator tests) - Test the mathematical formulas and builtin pipeline behavior in isolation
...
```bash
PYTHONPATH=components/src python -m pytest components/src/dynamo/planner/tests/unit/test_load_based_scaling.py components/src/dynamo/planner/tests/plugins/orchestrator -v
```
...
- **[kube-prometheus-stack](../../../../../../docs/fern/kubernetes/operations/observability.mdx) installed and running.** The SLA planner requires Prometheus to observe metrics and make scaling decisions.
- Ensure the Dynamo operator was installed with the Prometheus endpoint configured (see [SLA Planner Quickstart Guide](../../../../../../docs/fern/developer-guide/knowledge-base/modular-components/planner/planner-guide.md#prerequisites) for details).
...
- Config 4 with SLA planner using the builtin plugin pipeline:
 `./perf_test_configs/disagg_8b_planner.yaml`
