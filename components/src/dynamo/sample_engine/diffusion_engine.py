...
from dynamo.backend.health_check import build_raw_health_check_payload, is_probe
...
class SampleDiffusionEngine(DiffusionEngine):
    ...
    async def from_args(
        ...
        add_worker_args(
            parser, default_component="sample", default_endpoint_types="images"
        )
        ...
        worker_config = build_worker_config(
            ...
            # Raw media pipeline: the frontend forwards the request verbatim, so
            # there is no tokenizer stage; and diffusion has no KV cache to route on.
