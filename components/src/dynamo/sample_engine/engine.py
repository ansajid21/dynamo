...
from dynamo.backend.disagg import enforce_prefill_max_tokens, require_prefill_result
from dynamo.backend.health_check import build_health_check_payload, is_probe
from dynamo.backend.logprobs import parse_logprob_options
from dynamo.backend.multimodal import (
    encoder_terminal_chunk,
    extract_multimodal_kwargs,
    require_encoder_result,
)
from dynamo.backend.publisher import ComponentSnapshot, KvEventSource, PushSource
...
class SampleLLMEngine(LLMEngine):
    ...
    async def from_args(
        ...
        add_worker_args(parser, default_component="sample")
    ...
    async def generate(
        ...
        try:
            # Parent-chain pinned in test_backend_worker_otlp_export.
