...
class GenerateRequest(TypedDict, total=False):
    ...
    read them via ``dynamo.backend.disagg`` helpers.
    ...
    :func:`dynamo.backend.multimodal.require_encoder_result`. All
...
class GenerateChunk(TypedDict, total=False):
    ...
    :func:`dynamo.backend.multimodal.encoder_terminal_chunk`.
    """
...
class BaseEngine(ABC):
    ...
    async def register_prometheus(self, metrics: "EngineMetrics") -> None:
        ...
        no-op. See :mod:`dynamo.backend.metrics` for helpers. Do not
...
#: Env var that activates the built-in smoke hook on any Backend SDK.
