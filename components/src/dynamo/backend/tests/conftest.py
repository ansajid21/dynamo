...
Every test here imports ``dynamo.backend``, which loads the PyO3 extension
``dynamo._core.backend`` at import time. On a source checkout without
``maturin develop`` that import fails during collection, so ignore the whole
