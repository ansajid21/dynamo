...
from ._engine import BaseEngine, RawEngine
...
# supported by the Backend SDK abstraction.
...
def _to_rust_disaggregation_mode(mode: DisaggregationMode):
    try:
        ...
        raise NotImplementedError(
            f"DisaggregationMode.{mode.name} is not supported by the Backend "
            "SDK; use the legacy backend entry point for this worker role"
        ) from e
