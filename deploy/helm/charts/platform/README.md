...
event planes use TCP and ZMQ, respectively. This is a breaking change for existing releases that
previously relied on the chart's implicit `global.nats.install=true` default.

During an upgrade, accepting the new default removes the bundled NATS resources and removes
`NATS_SERVER` from operator-managed workload specifications. That environment change can trigger
rolling updates as those workloads are reconciled. Before disabling NATS, verify that the deployment
does not use an explicit NATS-based feature such as `DYN_EVENT_PLANE=nats`, legacy
`DYN_REQUEST_PLANE=nats`, or a NATS request-trace sink.

To retain bundled NATS and avoid a rolling update caused by removing `NATS_SERVER`, explicitly
preserve the previous setting during the upgrade:
...
Deployments using an external NATS server should continue to set `dynamo-operator.natsAddr` instead.
...
| global.nats.install | bool | `false` | Whether this chart should install the bundled NATS subchart. When true, deploys NATS and auto-configures the operator with its address. When false, NATS is not deployed. Use dynamo-operator.natsAddr to point at an external instance if you are bringing your own NATS. NATS is optional because the default request and event planes use TCP and ZMQ, respectively. |
...
respectively. The bundled NATS subchart is therefore **disabled by default**.

Enable bundled NATS when using an explicit NATS-based feature such as
`DYN_EVENT_PLANE=nats`, legacy `DYN_REQUEST_PLANE=nats`, or a NATS request-trace sink:
...
```yaml
dynamo-operator:
  natsAddr: "nats://my-external-nats:4222"
```
...
- [Dynamo Cloud Deployment Installation Guide](../../../../docs/fern/kubernetes/installation/install-dynamo.md)
